...
export class TuneView {
  ...
  private render(): void {
    ...
    this.navEl.appendChild(
      el("div", { class: "tv-nav-title", text: `${scene.title} \u00b7 ${scene.lines.length} lines` }),
    );
    scene.lines.forEach((ln, idx) => {
      ...
      const btn = el("button", {
        class: "tv-nav-row" + (isActive ? " active" : "") + (has ? " has-takes" : ""),
        title: ln.text,
      });
      btn.append(
        el("span", { class: "tv-nav-num", text: `${idx + 1}` }),
        el("span", { class: "tv-nav-text", text: ln.text || "(empty)" }),
        el("span", {
          class: "tv-nav-count",
          text: sect?.takes.length ? `${sect.takes.length}` : "\u2013",
        }),
      );
      btn.onclick = () => this.player.seek(this.player.offsets[this.player.sceneIndex] + ln.from + 0.001);
    ...
    if (!activeId) {
      this.bodyEl.appendChild(
        el("div", { class: "tv-empty", text: "seek to a script line to compare its takes" }),
      );
      return;
    }
    const activeLine = scene.lines.find((ln) => ln.id === activeId) as TimedText | undefined;
    const sect = this.takes.section(scene.id, activeId);
    const header = el("div", { class: "tv-body-head" });
    header.append(
      el("span", { class: "tv-body-time", text: activeLine ? `${fmt(activeLine.from)}\u2013${fmt(activeLine.to)}` : "" }),
    ...
    if (!sect?.takes.length) {
      this.bodyEl.appendChild(
        el("div", { class: "tv-empty", text: "no takes for this line yet \u2014 record in RECORD mode" }),
      );
      return;
    }
    ...
    sect.takes.slice().reverse().forEach((tk, i) => {
      ...
      play.onclick = () => this.takes.toggleAudition(scene.id, activeId, tk.file);
      const name = el("span", {
        class: "tv-take-name",
        text: `take ${ord}`,
        title: tk.file,
      ...
      const star = el("button", {
        class: "tv-star",
        text: tk.file === sect.candidate ? "\u2605 pick" : "\u2606 pick",
      ...
      const del = el("button", { class: "tv-del", text: "\u2715", title: "move take to trash" });
      del.onclick = async () => {
      ...
      const strip = new TakeStrip(this.takes, scene.id, activeId, tk.file, { height: 56 });
