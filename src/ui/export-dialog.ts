...
export class ExportDialog {
  ...
  private build(): void {
    ...
    const closeBtn = el("button", { class: "export-close", text: "\u2715", title: "close (the export keeps running)" });
    closeBtn.onclick = () => this.close();
    head.appendChild(closeBtn);

    /* form */
    this.fpsSel = el("select", {},
      el("option", { value: "15", text: "15 fps draft" }),
      el("option", { value: "30", text: "30 fps", selected: "" }),
      el("option", { value: "60", text: "60 fps" }),
    ) as HTMLSelectElement;

    const sceneBtn = el("button", { class: "export-start", text: "this scene" }) as HTMLButtonElement;
    const fullBtn = el("button", { class: "export-start", text: "full video" }) as HTMLButtonElement;
    sceneBtn.onclick = () => void this.start(this.player.scene.id);
    fullBtn.onclick = () => void this.start(null);
    this.startBtns = [sceneBtn, fullBtn];

    this.form = el("div", { class: "export-form" },
      el("label", { class: "export-row" },
      ...
      el("div", { class: "export-hint",
        text: "Frame-exact render via headless Chrome; picked takes are muxed in. Iterate with \"this scene\" at 15 fps." }),
    );

    /* progress */
    this.bar = el("i");
    this.progress = el("div", { class: "export-progress" },
      el("div", { class: "export-bar" }, this.bar),
    );
    this.status = el("div", { class: "export-status" });
    this.cancelBtn = el("button", { class: "export-cancel", text: "background", title: "close this dialog; the export keeps running" }) as HTMLButtonElement;
    this.cancelBtn.onclick = () => this.close();

    panel.append(head, this.form, this.progress, this.status, this.cancelBtn);
  ...
  private async poll(): Promise<void> {
    ...
    if (s.state === "rendering" || s.state === "starting") {
      ...
      this.setStatus(`${s.phase || "rendering"} \u00b7 frame ${s.frame}/${s.totalFrames} (${pct}%)`);
      this.setBadge(`exporting \u2026 ${pct}%`);
    } else if (s.state === "done") {
      this.stopPolling();
      this.isRunning = false;
      this.setProgress(100);
      this.setStatus(`\u2713 done \u2014 <a href="${s.output}" target="_blank">open MP4</a>`);
      this.setBadge("\u2713 export done", true);
      this.startBtns.forEach((b) => b.disabled = false);
    } else if (s.state === "error") {
      this.stopPolling();
      this.isRunning = false;
      this.setStatus("\u2717 export error: " + ((s.message || "").split("\n")[0]));
